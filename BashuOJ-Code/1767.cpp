...
struct node{int a,b;ll c;};
struct inf{int to;ll val;};
node rel[MAXN];
int n,m,q,K;
int prt[MAXN],dep[MAXN],p[MAXN][25];
ll val[MAXN],g[MAXN][25];
vector<inf>a[MAXN];
inline int getint()
{
	int num=0,bj=1;
...
int main()
{
	...
	for(ri i=1;i<=m;i++)
	{
		int a=getint(),b=getint();ll c=getll();
		rel[i]=(node){a,b,c};
	}
